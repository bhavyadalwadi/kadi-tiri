    ...
    <>
      ...
      <div className="position-absolute start-0 ms-2 z-3" style={{ top: '20px', width: '240px' }}>
        <div className="rounded-3 text-white px-3 py-2 shadow-lg" style={{
          ...
          <div className="small text-light mb-1">Players: {gameState.players.length}/4</div>
          <div className="small text-light mb-2">You are: {gameState.players.find(p => p.id === currentPlayerId)?.name}</div>
          <button 
      ...
      <div className="position-absolute start-0 ms-2 z-3" style={{ top: '177px', width: '240px' }}>
        <div className="rounded-3 text-white px-3 py-2 shadow-lg" style={{
          ...
          <div className="d-flex align-items-center mb-1">
            <span className="me-2 fs-6">👀</span>
            <strong className="text-white small">Player View</strong>
          </div>
          <div className="mb-1 small text-light" style={{ fontSize: '11px' }}>Control any player for testing</div>
      ...
      <div className="position-absolute start-0 ms-2 z-3" style={{ top: '330px', width: '240px' }}>
        <div className="rounded-3 text-white px-3 py-2 shadow-lg" style={{
          background: 'linear-gradient(135deg, rgba(220, 38, 38, 0.15), rgba(0, 0, 0, 0.8))',
          border: '1px solid rgba(220, 38, 38, 0.3)',
          backdropFilter: 'blur(10px)',
          ...
          <div className="mb-1 small">Turn: {gameState.players.find(p => p.id === gameState.currentPlayer)?.name || 'Unknown'}</div>
          {gameState.status === 'playing' && (
            <>
              <div className="mb-1 small">Powerhouse: <span className="text-warning fw-bold">{gameState.settings.powerhouseSuit}</span></div>
              <div className="mb-1 small">Tricks: {gameState.completedTricks.length}</div>
              <div className="small">Cards Left: {gameState.players.find(p => p.id === currentPlayerId)?.cards.length || 0}</div>
            </>
          )}
        </div>
      </div>
